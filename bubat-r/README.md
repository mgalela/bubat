...
Evidence-based reconstruction companion for BUBAT.
...
- optional feed into BUBAT stages `01–04`

## Boundary

BUBAT-R is not a BUBAT stage.

```text
existing codebase
→ BUBAT-R reconstruction
→ reference design + coverage + gaps + readiness
→ optional feed into BUBAT 01–04
```

BUBAT remains architecture staging/documentation pipeline.
BUBAT-R is forensic reconstruction/takeover layer.
...
BUBAT-R currently ships as an internal BUBAT module.
...
### Add templates to a target project

From BUBAT workspace:
...
These are command contracts for now; implementation can be added later.
...
Then follow `bubat-r/workflow.md`.
